...
wordDict = {3: ["sob", "vil", "não", "paz", "ser", "mal", "ver", "céu", "ter", "ego", "bem", "vão", "vir", "mãe",
                ...
                4: ["amor", "fato", "viés", "puta", "mito", "esmo", "caos", "brio", "vide", "como", "ação", "após",
                    ...
                    "sair", "tese", "irão", "ódio"],
                5: ["sagaz", "negro", "âmago", "êxito", "mexer", "termo", "algoz", "senso", "nobre", "plena", "afeto",
                    ...
                    "pífio"],
                6: ["exceto", "cínico", "idôneo", "âmbito", "néscio", "índole", "mister", "vereda", "apogeu", "inócuo",
                ...
                7: ["empatia", "embuste", "cônjuge", "exceção", "efêmero", "prolixo", "idílico", "caráter", "análogo",
                    ...
                    "mancebo"],
                8: ["inerente", "peculiar", "denegrir", "respeito", "anuência", "deferido", "genocida", "prudente",
                ...
                9: ["perspicaz", "recíproco", "impressão", "concessão", "supérfluo", "escrúpulo", "retificar",
...
def getRandomWordFromWordList(num_of_letters):
    wordList = wordDict[num_of_letters]
    ...
    wordDict[num_of_letters].remove(randWord)

    return randWord


def createAnagram(randWord):
    anagram = []
    indexes = []
    num = len(randWord)
    i = 0
...
def getRandomWordLocally(num_of_letters):
    random_word = getRandomWordFromWordList(num_of_letters)
    random_word_anagram = createAnagram(random_word)

    return { 'word': random_word, 'scrambledWord': random_word_anagram }
...
def getNewWord(data):
    ...
    if (data.get('getFromOpenAi')):
        language = data.get('language')
        word_data_dictionary = getRandomWordFromOpenAi(language, num_of_letters)
    else:
        word_data_dictionary = getRandomWordLocally(num_of_letters)

    used_words.get(str(num_of_letters)).append(word_data_dictionary.get('word'))
    print(used_words)
    return word_data_dictionary

# Returns a dictionary containing both a random word and that word scrambled.
# e.g. { 'word': 'cat', 'scrambledWord': 'tac' }
def getRandomWordFromOpenAi(language, num_of_letters):
    word_data = get_word_from_open_ai(language, num_of_letters)
    return eval(word_data)
